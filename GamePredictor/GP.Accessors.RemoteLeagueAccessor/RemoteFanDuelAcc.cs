...
namespace GP.Accessors.RemoteLeagueAccessor
{
    public interface IRemoteFanDuelAcc
    {
        ...
        bool NavigateToLeague(GPChromeDriver cachedDriver, FantasyLeagueEntry interestedLeague);
        ...
        bool ValidateLeague(GPChromeDriver cachedDriver, string url);
    }
    ...
    public class RemoteFanDuelAcc : IRemoteFanDuelAcc
    {
            ...
            foreach (var foreignPlayerId in data.Keys)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            availability = PlayerAvailability.a_Out;
                        }
                    }

                    if (availability == PlayerAvailability.a_Available
                        && (data[foreignId][10] == null
                            || string.IsNullOrWhiteSpace(data[foreignId][10].ToString()))) // ignore players with injuries
                    {
        ...
        public bool NavigateToLeague(GPChromeDriver cachedDriver, FantasyLeagueEntry interestedLeague)
        {
            ...
            try
            {
                var clearPreviousRoster = cachedDriver.FindElementById(@"playerPickerClearAllButton");
                if (clearPreviousRoster != null)
                {
                    clearPreviousRoster.Click();
                    cachedDriver.SwitchTo().Alert().Accept();
                }
            }
            catch { }
        ...
        public bool ValidateLeague(GPChromeDriver cachedDriver, string url)
        {
            ...
            //<li class="contest-entries-league">Entries: <b><a href="#" data-tab="entries" class="lightboxLeagueEntries_show">5</a></b> / 5<span class="entries-player-league"> player league</span></li>
